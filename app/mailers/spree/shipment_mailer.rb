module Spree
  class ShipmentMailer < ActionMailer::Base
    ...
    def notify(shipment)
      ...
      attachments['packaging_slip.pdf'] = render('spree/admin/documents/packaging_slip', format: :pdf)
      attachments['non_wood.pdf'] = render('spree/admin/documents/non_wood', format: :pdf) if %(AUS).include?(shipment.order.ship_address.country.try(:iso3))

      recipient = "Ultimaker Logistics <logistics@ultimaker.com>"
